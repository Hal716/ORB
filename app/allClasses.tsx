...
export default function AllClasses() {
  ...
  const handleEdit = (index: number) => {
    router.push(`/add-class?index=${index}` as any);
  };
    ...
    Alert.alert("Delete Class", "Are you sure you want to delete this class?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          const updatedClasses = classes.filter((_: any, i: number) => i !== index);
  ...
  const sortedClasses = [...classes].sort((a: Classes, b: Classes) => {
    ...
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView contentContainerClassName="pb-24 pt-5 px-5">
        ...
        {sortedClasses.length > 0 ? (
          sortedClasses.map((item: Classes) => {

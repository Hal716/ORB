...
function requireDatabaseConfig() {
  if (!APPWRITE_DATABASE_ID || !APPWRITE_CLASSES_TABLE_ID) {
    throw new Error(
      "Missing Appwrite classes config. Add EXPO_PUBLIC_APPWRITE_DATABASE_ID and EXPO_PUBLIC_APPWRITE_CLASSES_TABLE_ID to .env",
    );
  }
}
...
function requireTasksCollectionConfig() {
  if (!APPWRITE_DATABASE_ID || !APPWRITE_TASKS_TABLE_ID) {
    throw new Error(
      "Missing Appwrite tasks config. Add EXPO_PUBLIC_APPWRITE_TASKS_TABLE_ID to .env",
    );
  }
}
...
export async function fetchClassesFromAppwrite(userId) {
  ...
  return response.rows.map((row) => ({
    ...
    color: row.color ?? "#f5c542",
    appwriteRowId: row.$id,
  }));
}
...
export async function saveClassToAppwrite({
  ...
  console.log("Saving class to Appwrite:", {
...
export async function fetchTasksFromAppwrite(userId) {
...
export async function saveTaskToAppwrite({
  ...
  console.log("Saving task to Appwrite:", {
    databaseId: APPWRITE_DATABASE_ID,

...
export default function Addtask() {
  ...
  const scheduleTaskAlarm = async (task: Tasks, isoDate: string) => {
    // Request permissions
    const { status } = await Notifications.requestPermissionsAsync();
    if (status !== "granted") {
      Alert.alert("Permission denied", "Notifications permission is required for alarms.");
      return null;
    }
  
    // Cancel existing notification for this task if editing
    if (editingIndex !== null && TASKS[editingIndex]?.notificationId) {
      await Notifications.cancelScheduledNotificationAsync(
        TASKS[editingIndex].notificationId!
      );
    }
  
    const triggerDate = dayjs(isoDate).subtract(30, "minute").toDate();
  
    // Don't schedule if trigger time is in the past
    if (triggerDate <= new Date()) {
      Alert.alert("Notice", "Due date is too soon to set a 30-minute reminder.");
      return null;
    }
  ...
  const handleSave = async () => {
    if (!name.trim() || !subject.trim() || !dueDate.trim()) {
      ...
      return; // 👈 this was missing, and the closing } below was removed
    }
    ...
    const notificationId = await scheduleTaskAlarm(newTask, builtDate);
    if (notificationId) {
